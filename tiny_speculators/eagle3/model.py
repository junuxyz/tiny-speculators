...
class Eagle3DraftModel(PreTrainedModel):
    ...
    def __init__(
        ...
        self.register_buffer("d2t", draft_to_target_offsets.to(dtype=torch.long))

        self.register_buffer("t2d", target_to_draft.to(dtype=torch.bool))
        ...
        # d2t
        recovered_target_ids = self.d2t + torch.arange(
            self.d2t.numel(),
    ...
    def forward(
        ...
        with torch.no_grad():
            targets = self.lm_head(self.verifier_norm(verifier_last_hidden_states))
        ...
        for ttt_step in range(self.config.ttt_steps):
            ...
            if ttt_step == 0:
                ...
                step_logits = logits[
                    :, :-ttt_step
                ]  # exclude logits that exceed ttt_step
                step_targets = targets[:, ttt_step:]
                step_loss_mask = loss_mask[:, ttt_step:]
                same_document = doc_ids[:-ttt_step] == doc_ids[ttt_step:]
