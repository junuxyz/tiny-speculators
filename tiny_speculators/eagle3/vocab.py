...
def load_vocab_mapping(path: Path) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    draft_to_target_offsets = mappings["draft_to_target_offsets"].to(
        dtype=torch.long
    )
...
def build_vocab_mappings(
    ...
    if not 0 < draft_vocab_size <= target_vocab_size:
        raise ValueError(
            f"draft_vocab_size must be between 1 and {target_vocab_size}"
        )
    
    sorted_tokens = sorted(
        token_frequencies,
        key=lambda tid: (-token_frequencies[tid], tid)
    )
    ...
    draft_to_target_offsets = (
                        torch.tensor(selected_token_ids, dtype=torch.long)
